                ...
                if myJob ~= 'police' and myJob ~= 'sheriff' and myJob ~= 'state' and myJob ~= 'ambulance' then
                    if sementeQTE >= 1 then
                        if plantado[i] == 0 then
                            ...
                            CurrentActionMsg  = 'Press ~INPUT_CONTEXT~ to plant marijuana seeds.'
                            CurrentActionData = {}
                        end
                    end
                end
            ...
            if zone == 'WeedFarmFarm1' then
                if myJob ~= 'police' and myJob ~= 'sheriff' and myJob ~= 'state' and myJob ~= 'ambulance' then
                    if pronto[i] == 1 then
                        ...
                        CurrentActionMsg  = 'Press ~INPUT_CONTEXT~ to start growing.'
                        CurrentActionData = {}
                    end
                end
            end
...
AddEventHandler("weedz:sucessfarm", function(Spot)
    pronto[Spot] = 0
    CurrentActionMsg  = 'Press ~INPUT_CONTEXT~ Take the weed.'
    CurrentActionData = {}
	isInMarker = false
	hasAlreadyEnteredMarker = false
            ...
            if(GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < Config.DrawDistance) then
			    ...
			    if plantado[2] == 1 then
				    if tempo[2] > 0 then
				        ...
				        DrawText3D(v.x, v.y, v.z - 0.6, "Take Weed")
				    end
				else
				    DrawText3D(v.x, v.y, v.z - 0.6, "Plant Here")
				end
