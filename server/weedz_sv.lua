...
AddEventHandler("weedz:water", function(water, plantanr)
	...
	--if acquaQTY ~= 0 then
		...
		sendToDiscord('📝 Log Weed', 'Il giocatore __**' .. playerName .. '**__ You watered the plant')
		for i=1, #dados, 1 do
	...
	--else
		--TriggerClientEvent('weedz:notification', source, 'You have no more water.')
	--end
end)
...
AddEventHandler("weedz:fertilizer", function(fertilizer, plantanr)
	...
	--if fertilizzateQTY ~= 0 then
		local dados   = MySQL.Sync.fetchAll('SELECT * FROM weed')
		--TriggerClientEvent('weedz:notification', source, 'Added fertilizer.')

		xPlayer.removeInventoryItem('fertilizzante', 1)
		sendToDiscord('📝 Log Weed', 'Il giocatore __**' .. playerName .. '**__ You used the fertilizer')
		for i=1, #dados, 1 do
			if dados[i].Spot == plantanr then 
				if dados[i].Ready == 0 then
	...
	--else
	--	TriggerClientEvent('weedz:notification', source, 'You have no more fertilizer.')
	--end
end)
	...
	SetTimeout(10000, function()
		...
		if PlayersPlantandoWeeds[source] == true then
			...
			--if seedQTY > 0 then
				...
				TriggerClientEvent('weedz:notification', source, 'Planted Successfully.')
				...
				sendToDiscord('📝 Log Weed', 'Il giocatore __**' .. playerName .. '**__ You planted a seed°' .. spt)
			--else
			--	TriggerClientEvent('weedz:notification', source, 'You have no more seeds.')
			--end
		end
	end)
end 
...
AddEventHandler('weedz:startPlantarWeed', function(Spot)
	...
	if LockPlantar[Spot] == nil then
		...
		--if seedQTY > 0 then
			...
			TriggerClientEvent('weedz:notification', _source, 'You are planting...')
	    ...
	    TriggerClientEvent('weedz:notification', source, 'Someone is already using this spot!')
	end
end)
...
function ApanhaWeed(source, spt)
	...
	SetTimeout(10000, function()
		...
		if PlayersApanhandoWeeds[source] == true then
			...
			local xPlayer = ESX.GetPlayerFromId(source)
			local grammiQTY = xPlayer.getInventoryItem('weed')
			local playerName = GetPlayerName(source)

			if grammiQTY.limit ~= -1 and grammiQTY.count >= grammiQTY.limit then
				TriggerClientEvent('weedz:notification', source, 'You have to much weed in your inventory')
				PlayersApanhandoWeeds[source] = false
				LockApanha[spt] = nil
			else
				xPlayer.addInventoryItem('weed', 30)
				TriggerClientEvent('weedz:notification', source, 'Weed successfully cultivated')
				PlayersApanhandoWeeds[source] = false
				...
				sendToDiscord('📝 Log Weed', 'Il giocatore __**' .. playerName .. '**__ You have finished harvesting the weed n°' .. spt)
				LockApanha[spt] = nil
			end
...
AddEventHandler('weedz:startApanhaWeed', function(Spot)
	...
	if LockApanha[Spot] == nil then
	    ...
	    TriggerClientEvent('weedz:notification', source, 'You are taking the weed...')
